...
// Example function to match users in a session 2 logic
function matchUsersInSession(user1, user2) {
...
function sendTextMessage(message) {
    if (socket.readyState === WebSocket.OPEN) {
        const data = { type: 'text-message', message, sender: 'You', userId };
...
// Example function to display a text message in the chat box
...
socket.addEventListener('message', (event) => {
    ...
    switch (data.type) {
        case 'text-message':
            ...
            break;
        case 'matched-users':
            matchUsersInSession(data.user1, data.user2);
            break;
